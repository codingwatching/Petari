...
void MarioActor::initCommonEffect() {
    ...
    while (entry->mName) {
        ...
        const char* flowerEffects[] = { lbl_805C70F8, lbl_805C7105, lbl_805C70F8, lbl_805C7105 };
        const char* sandEffects[] = { lbl_805C7116, lbl_805C7121, lbl_805C7116, lbl_805C7121 };
        const char* snowEffects[] = { lbl_805C7130, lbl_805C713B, lbl_805C714A, lbl_805C7155 };
        const char* smokeFollowEffects[] = { lbl_805C7164, lbl_805C7176, lbl_805C718D, lbl_805C719F };
        const char* mudEffects[] = { lbl_805C71B5, lbl_805C71BF, lbl_805C6A7F, lbl_805C71CD };
        const char* honeyEffects[] = { lbl_805C71DB, lbl_805C71E7, lbl_805C6A89, lbl_805C71F7 };
        ...
        for (u32 materialIndex = 0; materialIndex < 7; materialIndex++) {
            ...
            if (flag2 != 0) {
                ...
                case 1:
                    addMarioEffectAtJoint(this, effectName, base + 0x7D7, name + materialIndex);
                    break;
                case 2:
                    addMarioEffectAtJoint(this, effectName, base + 0x7DD, name + materialIndex);
                    break;
                case 3:
                    addMarioEffectAtJoint(this, effectName, base + 0x7E3, name + materialIndex);
                    break;
                default:
                    break;
                }
...
void MarioActor::updateEffect() {
    ...
    if (effectC) {
        playEffectRT(base + 0x1145, mMario->_368, mMario->mShadowPos);
    } else {
        if (_B98 & 0x04000000) {
...
void MarioActor::initEffect() {
    ...
    mEffectKeeper->registerEffect(base + 0xF04, pos, rot, scale, base + 0xEF5);
    mEffectKeeper->registerEffect(base + 0xF24, pos, rot, scale, base + 0xF1B);
    mEffectKeeper->registerEffect(base + 0xEE8, pos, rot, scale, base + 0xF33);
    mEffectKeeper->registerEffect(base + 0xF50, pos, rot, scale, base + 0xF46);
    mEffectKeeper->registerEffect(base + 0xF7A, pos, rot, scale, base + 0xF65);
    mEffectKeeper->registerEffect(base + 0xF8F, _DAC.toMtxPtr(), base + 0xF82, nullptr);
    mEffectKeeper->registerEffect(base + 0xFB7, pos, rot, scale, base + 0xFAA);
    mEffectKeeper->registerEffect(base + 0xFD3, pos, rot, scale, base + 0xFC8);
    mEffectKeeper->registerEffect(base + 0xFEE, pos, rot, scale, base + 0xFE1);
    ...
    mEffectKeeper->registerEffect(base + 0x10D2, pos, rot, scale, base + 0x10C3);
    MR::getEffect(this, base + 0x10C3)->forceFollowOn();
    mEffectKeeper->registerEffect(base + 0x10F1, pos, rot, scale, base + 0x10E2);
...
void MarioActor::emitEffectWaterColumn(const TVec3f& rDir, const TVec3f& rTrans) {
    ...
    _BA8->appear();
    MR::startBck(_BA8, lbl_805C7B31, nullptr);
    MR::startBrk(_BA8, lbl_805C7B31);
    ...
    MR::emitEffect(_BA8, lbl_805C7B31);
